...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        ...
        self.train_iterator = list(get_batch_iterator(**self.data_iterator_kwargs, split='train', n_epochs=config.n_epochs, n_examples=config.n_examples, batch_size=config.batch_size, silent=rank != 0, cache_dir="misc/imdb_rlhf_pairs.csv"))
        rank0_print(f'Loaded train data iterator')
        self.eval_iterator = list(get_batch_iterator(**self.data_iterator_kwargs, split='test', n_examples=config.n_eval_examples, batch_size=config.eval_batch_size, silent=rank != 0, cache_dir="misc/imdb_rlhf_pairs.csv"))
